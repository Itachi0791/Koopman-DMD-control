...
load("DataFiles\softrobot_sim_data.mat") % Load stored trajectory data

N_sim = size(moments,1);  % Number of trajectories
N_traj = size(moments,2); % Number of time steps
n = size(states,3);       % State dimension
m = size(moments,3);      % Input dimension
delt = 0.1;              % Time step size (if needed)
nD = 0;                     % Delay depth
x_bar = 0.3;y_bar=0.3;
%y_bias = min(min(st))

traj_data = cell(1,N_sim);
for s = 1:N_sim
    % Extract trajectory data
    traj_data{s}.t = 0:delt:(N_traj-1)*delt;
    traj_data{s}.x = [zeros(n,1), squeeze(states(s,1:N_traj-1,:))']; % (n x N_traj)
    traj_data{s}.x = max(min(traj_data{s}.x,x_bar),-2*x_bar);
    traj_data{s}.y = squeeze(states(s,:,:))';  % (n x N_traj)
    traj_data{s}.y = max(min(traj_data{s}.y,y_bar),-2*y_bar);
    traj_data{s}.u = squeeze(moments(s,:,:))'; % (m x N_traj)
end
...
tic
W = [Ylift ; X];
lambda = 1e-3;  % Regularization parameter (adjust empirically)
V = [Xlift; U];  % Combined state-input matrix
WVt = [Ylift; X] * V';
VVt_reg = V * V' + lambda * eye(size(V, 1));  % Regularized Gram matrix
ABC = WVt / VVt_reg;  % Solve regularized least squares
...
close all;
traj_id = 1; % Trajectory to test
n_traj = 19; % Number of time steps to simulate
...
% Plot results
deltaT = 0.1; % Time step size (in seconds)
figure
lw_koop = 2; % Line width for plotting
subplot(1, 2, 1);
plot((0:n_traj) * deltaT, x_val_true, '-b', 'linewidth', lw_koop); hold on
plot((0:n_traj) * deltaT, x_val_pred, '--r', 'linewidth', lw_koop)
...
subplot(1, 2, 2);
plot((0:n_traj) * deltaT, y_val_true, '-b', 'linewidth', lw_koop); hold on
plot((0:n_traj) * deltaT, y_val_pred, '--r', 'linewidth', lw_koop)
...
Tmax = 50 * n_setpoints; % Total simulation time
% Initial condition (start near bottom position)
x0 = [0;0]; 
Nsim = Tmax / deltaT;   % Total number of simulation steps

% Generate random setpoints for pos_ref
pos_refs = [0.5;0.5];%2*pi * (rand(2, n_setpoints) - 0.5); % Random values in range [-pi, pi]

% Calculate duration of each setpoint
duration = Nsim / n_setpoints;

% Create yrr
tspan = linspace(0,Tmax,Nsim+1);
...
pos_ref_trajectory = repelem(pos_refs, 1, duration); % Repeat each setpoint for 'duration' steps

% Combine into yrr
yrr = pos_ref_trajectory;
...
Q = diag([100,100]); % Position tracking weight
QN = diag([1000, 1000]); % Higher terminal weight
R = diag([1,1,1,1,1]);
...
% Constraints
u_min = -5*ones(5,1); u_max = 5*ones(5,1);
pos_min = -0.5; pos_max = 0.5;

xlift_min = [pos_min;pos_min;-inf*ones(Nlift-n,1)];
xlift_max = [pos_max;pos_max; inf*ones(Nlift-n,1)];
...
if nD ~= 0
    zeta0 = [x_koop ; NaN(nD*(ny+m),1)];
    for i = 1:nD
        upast = zeros(m,1);
        xp = f_ud(0,x_koop,upast);
        zeta0 = [xp ; upast ; zeta0(1:n_zeta-ny-m)]; 
        x_koop = xp;
    end
    x0 = x_koop;
end
...
for i = 0:Nsim-1
    ...
    xlift_next = Alift*xlift + Blift*u_koop;
    x_koop = Clift*xlift_next;
...
figure;
subplot(2,1,1)
plot(tspan,XX_koop(1,:)); hold on;
plot(tspan(1:end-1),yrr(1,:))

subplot(2,1,2)
plot(tspan,XX_koop(2,:)); hold on;
plot(tspan(1:end-1),yrr(2,:))
%%
