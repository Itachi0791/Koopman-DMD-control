...
num_basis = 20; phi = cell(1,num_basis);
...
for i = 0:num_basis-1
    for j = 0:num_basis-1
        ...
        for k = 0:num_basis-1
            for q = 1 : floor((i+1)/2)
                sum = sum + f_coeffs(k+1)*sqrt(2*k+1)*(2*i-4*q+3)*Wigner3j([i-2*q+1,k,j],[0,0,0]).^2;
            end
        end
        A_bar_est(i+1,j+1) = (i==j) + delt*sqrt((2*i+1)*(2*j+1)/2)*sum;
    end
end
...
phi_x_prime = phi_x;
x_euler = zeros(1,Ns); x_euler(1)=x0;x_pred=zeros(1,Ns); x_pred(1) = x0;
%[t, x] = ode45(@(t,x) f(x), 0:delt:(Ns-1)*delt, x0);
C_x = zeros(1,num_basis); C_x(2) = sqrt(2/3); % Mapping from phi_1,phi_2,phi_3 to x
for i=1:Ns-1
    x_euler(i+1) = F(x_euler(i),delt);
    phi_x(:,i+1) = A_bar_est*phi_x(:,i);
    x_pred(i+1) = C_x*phi_x(:,i+1);
    for j=1:num_basis
        phi_x(j,i+1) = phi{j}(x_pred(i+1));
    end
    
   % phi_x_prime(:,i+1) = A_bar_prime*phi_x_prime(:,i);
end
[t, x_ode45] = ode45(@(t,x) f(x), 0:delt:(Ns-1)*delt, x0);
...
hold on;
grid on;
plot(0:delt:(Ns-1)*delt,x_ode45,"LineWidth",3.5,"Color",[0 1 0])
plot(0:delt:(Ns-1)*delt,x_euler,"LineWidth",3.5,"Color",[0 0 1])
...
title("Time evolution prediction($\Delta t = 0.1$)","interpreter","latex","FontSize",25)
legend("ODE45","Euler","Predicted (using $\bar{A}$)","interpreter","latex")
